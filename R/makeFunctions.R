...
#' 
#' @details Cretae a matrix of dimension n by m which contains a normalised (unit energy in each column) set of blur functions of regular smooth type, generated by a Gamma density. These Gamma densities are generated using the \code{\link{dgamma}} base R function and then normalised to have unit energy.
#' 
...
#' 
#' @param n An integer specifying the number of observations in each channel
#' @param BA A numeric vector specifying box car widths for each channel.
#' 
#' @description Create blur of the box car type.
#' 
#' @details Creates a matrix of dimension n by m which contains a normalised (unit energy in each column) set of box car blur functions. The generated box-car functions are governed by an input vector \code{BA} which specifies the box car width.
#' 
...
#' n <- 128
#' m <- 3
#' BA <- 1/sqrt(c(89,121,87))
#' 
#' blurMat <- boxcarBlur(n, BA)
#' matplot(blurMat, type = 'l', main = paste('Set of box car blur functions'))
#' 
#' signal = makeLIDAR(n)
#' t = (1:n)/n
#' matplot(t, signal, type = 'l', main = 'LIDAR test signal')
...
#' @title Determine noise scale levels from a required \bold{S}ignal to \bold{N}oise \bold{R}atios
...
#' @details The elements of sigma is governed by the blurred \bold{S}ignal-to-\bold{N}oise \bold{R}atio (SNR) measured in decibels (dB) where,
#' \deqn{SNR  = 10 log_{10} (||k*f||^2/sigma^2)}
#' and k*f is the blurred signal, ||.|| is the norm operator and sigma is the standard deviation of the noise. Roughly speaking, is considered high, medium and low for the cases 10 dB, 20 dB and 30 dB respectively.
#' @return A numeric vector, sigma of length m, giving the scales (channel level noise standard deviations) to achieve the desired SNR.
